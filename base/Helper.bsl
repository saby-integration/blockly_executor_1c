...
Функция ЭтоОбъектМетаданных(Данные)
	ТипДанных = Новый Массив;
	ТипДанных.Добавить(Тип("Строка"));
	ТипДанных.Добавить(Тип("Дата"));
	ТипДанных.Добавить(Тип("Число"));
	ТипДанных.Добавить(Тип("Булево"));
	ТипДанных.Добавить(Тип("Структура"));
	ТипДанных.Добавить(Тип("Соответствие"));
	Если ТипДанных.Найти(ТипЗнч(Данные)) = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Функция block_obj_get_path_value(Знач obj, obj_path, obj_name)  Экспорт
	...
	Пока i < size Цикл
		РезультатЗначения = SabyПроверкаЗначения(_obj, size, i, _path, obj_path, obj_name, obj);
...
Функция SabyПроверкаЗначения(_obj, size, i, _path, obj_path, obj_name, obj) 
	...
	Попытка
		#Если Клиент И НЕ ТолстыйКлиентОбычноеПриложение Тогда
			Если ЭтоОбъектМетаданных(_obj) Тогда
				_obj = get_prop_onserver(_obj, elem);
			Иначе
...
Процедура ПреобразоватьСтруктутуКДопустимымXDTOТипам(Переменные)
	...
	Для Каждого Стр Из Переменные Цикл
		...
		Если Найти(Строка(Стр.Значение),"ТабличнаяЧастьСтрока")
			Или Найти(Строка(Стр.Значение),"TabularSectionRow")
			Тогда //И СтрДлина(ТипЗнч(Стр.Значение)) > 6 Тогда
			Переменные[Стр.Ключ] = ПолучитьСтруктуруИзСтрокиТабличнойЧасти(Стр.Значение);					
		КонецЕсли;
		
		Если Найти(НРег(ТипЗнч(Стр.Значение)),"объект")
			Или Найти(НРег(ТипЗнч(Стр.Значение)),"object")
			Тогда
		...
		Если Строка(Стр.Значение) = "СтрокаТаблицыЗначений" 
			Или Строка(Стр.Значение) = "ValueTableRowТогда" Тогда
...
Функция ПолучитьСтруктуруИзОбъекта(знач Объект)
	...
	КонецЦикла;    
	Структура.Вставить("ДополнительныеСвойства", Новый Структура);
	Возврат Структура;
КонецФункции
...
Функция ПолучитьСтруктуруИзСтрокиТабличнойЧасти(знач Объект)
	...
	ПутьТЧ		= Saby_СтрРазделить82(СтрокаОбъект, ".", Ложь);
...
Процедура ПроверитьНаличиеОбязательныхПараметров(СтруктураИсточник, ИменаКлючей, ПерфиксОшибки = "")
	...
	МассивИменПараметров = Saby_СтрРазделить82(ИменаКлючей,",");
	ТекстПараметров = "";
...
Функция Saby_ПривестиПеременнуюКДопустимойНаКлиенте(Переменная) 
	Если ТипЗнч(Переменная) = Тип("Массив") Тогда 
		Для Каждого Запись Из Переменная Цикл
			Запись = Saby_ПривестиПеременнуюКДопустимойНаКлиенте(Запись);	
		КонецЦикла;	
	ИначеЕсли ТипЗнч(Переменная) = Тип("Соответствие") Или ТипЗнч(Переменная) = Тип("Структура") Тогда
		Для Каждого Запись Из Переменная Цикл 
			Переменная.Вставить(Запись["Ключ"], Saby_ПривестиПеременнуюКДопустимойНаКлиенте(Запись["Значение"]));	
		КонецЦикла;	 
	ИначеЕсли ЭтоТипТабличнаяЧасть(Переменная) Тогда
	 	Возврат ПреобразоватьТаблицуЗначенийВМассивСтруктур(Переменная.Выгрузить());
	ИначеЕсли ТипЗнч(Переменная) = Тип("ТаблицаЗначений") Тогда
	 	Возврат ПреобразоватьТаблицуЗначенийВМассивСтруктур(Переменная);	
	Иначе
		Возврат Переменная;
	КонецЕсли;
	Возврат Переменная;
