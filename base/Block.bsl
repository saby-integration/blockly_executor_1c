Функция block_execute(block_type, node, path, context, block_context)
	...
	Попытка  
		БлокВыполнен = Ложь;
		result = block_execute_core(block_type, node, path, context, block_context, БлокВыполнен);
		Если НЕ БлокВыполнен Тогда 
			result = block_execute_SABY(block_type, node, path, context, block_context, БлокВыполнен);	
		КонецЕсли;
		Если НЕ БлокВыполнен Тогда 
			result = block_execute_c1(block_type, node, path, context, block_context, БлокВыполнен);	
		КонецЕсли;
		Если НЕ БлокВыполнен Тогда 
			result = simple_block_execute(block_type, node, path, context, block_context, БлокВыполнен);	
		КонецЕсли;
	Исключение
...
Функция block_execute_core(block_type, node, path, context, block_context, БлокВыполнен) 
	БлокВыполнен = Истина;
	...
	ИначеЕсли block_type = "controls_whileUntil" Тогда
		Возврат block_controls_while_until_execute(block_type, node, path, context, block_context);
	ИначеЕсли block_type = "controls_forEachAsync" Тогда
		Возврат block_multithreadloop_execute(block_type, node, path, context, block_context, Истина);
	Иначе
		БлокВыполнен = Ложь;
	КонецЕсли;
...
Функция block_execute_SABY(block_type, node, path, context, block_context, БлокВыполнен) 
	БлокВыполнен = Истина;
