...
//  uuid_default - Строка - uuid по умолчанию (используется ключ фонового задания при вызове ExecuteOperation, чтобы uuid можно было сразу вернуть в браузер)
//
...
Функция report_init(operation = Неопределено, uuid = Неопределено, service = Неопределено,
		Знач action_to_write = Неопределено, algorithm_name = "", uuid_default = Неопределено)
	...
	Если Не ЗначениеЗаполнено(uuid) Тогда
		Если ЗначениеЗаполнено(uuid_default) Тогда
			operation_uuid = uuid_default;	
		Иначе
...
Функция extsyncdoc_params(status_esd = Неопределено)
	data_operation = get_prop(context.operation, "data", Неопределено);
	Робот = get_prop(data_operation, "isRobot", Ложь);
...
Процедура report_update(status_esd = Неопределено)
	status = context.status;
	
	Если Не context.report.new И (status = "complete" Или status = "error") Тогда
...
Процедура report_add_objects(objects, run = Ложь)
    ...
    КонецЕсли;    
	Если context.report.new ИЛИ context.report.objects_to_write.Количество() > 20 Тогда
		report_write_unregistered_objects(extsyncdoc_params(), run);
	КонецЕсли;
КонецПроцедуры

//DynamicDirective

Процедура report_write_unregistered_objects(extsyncdoc_params, run = Ложь)
