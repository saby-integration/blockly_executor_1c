...
Функция blockly_executor_execute(ДокументDOM, endpoint = Неопределено) Экспорт
	...
	Исключение
		...
		Возврат СформироватьОшибку(ОшибкаСтруктура);		
	КонецПопытки;
	ПреобразоватьСтруктутуКДопустимымXDTOТипам(context_variables_get(context));
	Возврат Новый Структура("Result, CurrentVariables, Status", result, context_variables_get(context), "complete");
КонецФункции 

//DynamicDirective
Функция СформироватьОшибку(ОшибкаСтруктура)
	Если ЭтоСлужебнаяОшибкаБлока(ОшибкаСтруктура.type) Тогда
		...
		КонецЕсли;
		Возврат Новый Структура ("CurrentBlock, CurrentVariables, Result, Status, Commands, CommandsData", Currentblock_context.__id, context_variables_get(context), Currentblock_context, "run", context.commands, commands_data); 			
	Иначе
