...
// blockly_executor_init
//
// Параметры:
//  logger - Неопределено - logger
//  plugins - Неопределено - data
//  breakpoints - Неопределено - breakpoints
//
...
// blockly_executor_init_workspace
//
// Параметры:
//  data - Структура - data
//  name - Строка - Имя
...
// blockly_executor_execute
//
// Параметры:
//  ДокументDOM - ДокументDOM - Общий контекст алгоритма
...
Функция blockly_executor_execute(ДокументDOM, endpoint = Неопределено) Экспорт
	...
	Попытка
		...
		context.Вставить("result", result);
		Если ТипЗнч(contexts) <> Тип("Массив") Или contexts.Количество() = 0 Тогда 
			ЗаписатьПубличныеПараметры(context);
		КонецЕсли;
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		Возврат СформироватьОшибку(ИнфОбОшибке);		
	КонецПопытки;
	ПреобразоватьСтруктутуКДопустимымXDTOТипам(context_variables_get(context));
	context.Вставить("status", "complete");
	Возврат context_to_result();
КонецФункции 

//DynamicDirective

Процедура ДополнитьДействиями(context, Результат)
	Actions = get_prop(context.report, "actions_to_write");
	Если ЗначениеЗаполнено(Actions) Тогда
		Результат.Вставить("Actions", Actions);
	КонецЕсли;
КонецПроцедуры	

//DynamicDirective

Функция Saby_ДанныеКоманд(context)
	...
	Для Каждого command Из context.commands Цикл
		...
		КонецЕсли;
	КонецЦикла;	
	Возврат ДанныеДляКоманд;
КонецФункции

//DynamicDirective

Функция СформироватьОшибку(ИнфОбОшибке)
	...
	Если ЭтоСлужебнаяОшибкаБлока(ОшибкаСтруктура.type) Тогда
		...
		ДанныеДляКоманд = Saby_ДанныеКоманд(context);
		Если ОшибкаСтруктура.type = "DeferredOperation" Тогда  
			ОшибкаСтруктура.dump.Вставить("multi_thread_loop_block_context", Saby_КопияСтруктуры(context.block_context));			
			ОшибкаСтруктура.dump.Вставить("multi_thread_loop_variable_scopes", Saby_КопияМассива(context.variable_scopes, context.variable_scope_current + 1));	
		...
		Если ОшибкаСтруктура.type = "StepForward" Тогда
			...
			Возврат context_to_result(); 			
		КонецЕсли;
		Результат = Новый Структура ("ContextId, CurrentBlock, CurrentVariables, Result, Status, Commands, CommandsData", 
				context.uid, Currentblock_context.__id, context_variables_get(context), Currentblock_context, "run", context.commands, commands_data); 			
	ИначеЕсли ЭтоПроблемаСДоступомКСервису(ОшибкаСтруктура.type) Тогда			
		ВызватьИсключение ИнфОбОшибке.Описание;
	Иначе
		context.Вставить("status", "error");
		Результат = Новый Структура("ContextId, Result, Status", get_prop(context, "uid"), ОшибкаСтруктура, "error");
	КонецЕсли;
	ДополнитьДействиями(context, Результат);
	Возврат Результат;
КонецФункции


// blockly_executor_execute_nested
//
// Параметры:
//  endpoint - Строка - Точка входа
//  context - Структура - Общий контекст алгоритма
//  commands_result - Структура - commands result
//
// Возвращаемое значение:
//  - Структура - Результат обсчета алгоритма
//
...
Функция blockly_executor_execute_nested(context, endpoint, commands_result)
	workspace_raw = context_workspace_read(context);
...
// blockly_executor_execute_deferred
//
// Параметры:
//  endpoint_node - Строка - Точка входа
//  context - Структура - Общий контекст алгоритма
//
// Возвращаемое значение:
...
Функция blockly_executor_execute_deferred(endpoint_node, context)
	counter_deferred = context.deferred.Количество() - 1;
	_delete = Новый Массив;
	Для Сч = 0 По counter_deferred Цикл 
		_deferred = context.deferred[Сч];
		//Кузьмин		
        _context = context_init_deferred(context, _deferred);
        Попытка
            block_execute(endpoint_node.type, endpoint_node.node, "", _context, _context.block_context);
			context_restore_after_deferred(context, _context);
			_delete.Добавить(_deferred);
        Исключение 
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			Если ЭтоСлужебнаяОшибкаБлока(ОшибкаСтруктура.type) Тогда
				Если ОшибкаСтруктура.type = "DeferredOperation" Тогда
					ОшибкаСтруктура.dump.Вставить("uuid", _deferred["__deferred"]);
					context_add_deferred(context, ОшибкаСтруктура.dump);            
					// Перекладываем команды из временного контекста в основной
					ИнтеграцияДополнитьМассив(context.commands, _context.commands); 
					Продолжить;
		...
		КонецПопытки
	КонецЦикла;  
	// Удаляем выполненные отложенные операции
	Пока counter_deferred >= 0 Цикл 
		_deferred = context.deferred[counter_deferred];
		Если _delete.Найти(_deferred) <> Неопределено Тогда 
			context.deferred.Удалить(counter_deferred); 
		КонецЕсли;	
		counter_deferred = counter_deferred - 1;
	КонецЦикла;
