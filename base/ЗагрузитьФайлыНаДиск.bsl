...
Функция ЗагрузитьВременныеФайлыНаДискСБИС(Знач context_params, Знач ДанныеДляЗагрузки) Экспорт
	...
	Если НЕ context_params.Свойство("session") Тогда
		ТранспортИнтеграции.local_helper_auth_by_login(context_params);
	КонецЕсли;
	...
	Для каждого Вложение Из ДанныеДляЗагрузки Цикл
		...
		ЗаголовкиHTTP.Вставить("Content-Disposition", "attachment;filename*=UTF-8''"+ КодироватьСтрокуНаСервере(Файл["Имя"], ПредопределенноеЗначение("СпособКодированияСтроки.КодировкаURL"),"UTF-8"));
