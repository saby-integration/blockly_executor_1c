...
// Инициализация BlocklyContext
//
// Параметры:
//  uid - Соостветствие - uid
//  debug_mode - Соостветствие - debug_mode
//  current_block - Соостветствие - current_block
//  current_workspace - Соостветствие - current_workspace
//  workspace_name - Строка - workspace_name
//  workspace_own - Строка - workspace_own
//  workspace_file_type - Строка - workspace_file_type
//  data - Строка - data
//  report - Строка - report
//  service - Строка - service
//  clients - Строка - clients
//
...
Функция BlocklyContext_init(uid = Неопределено, debug_mode = Неопределено, current_block = Неопределено, current_workspace = Неопределено, workspace_name = Неопределено,
                   workspace_own = Неопределено, workspace_file_type = "xml", data = Неопределено, report = Неопределено, service = Неопределено, clients = Неопределено)
	...
	_context.Вставить("clients", clients);
    Возврат _context;
КонецФункции
...
Функция new_context()
	...
	_context.Вставить("is_deferred", Неопределено);
	_context.Вставить("variable_scope_deferred_index", Неопределено);
	_context.Вставить("variable_scope_current", 0);
...
//
// Параметры:
//  block_context - Соостветствие - block_context
//  workspace_name - Строка - workspace_name
//
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
...
Функция context_init_nested(block_context, workspace_name)
	...
	_self.operation.Вставить("operation_uuid",context.operation.operation_uuid);
...
// context_workspace_read
//
// Параметры:
//  context - Соостветствие - Контекс алгоритма
///
// Возвращаемое значение:
//  - ДокDOM - Результат обсчета алгоритма
//
//DynamicDirective
Функция context_workspace_read(context)
	Load_ini(context.workspace_name);
	ДокDOM = ПолучитьДокументDOM(IniGet(context.workspace_name));
	Возврат ДокDOM;
КонецФункции

// context_init_deferred
//
// Параметры:
//  context - Структура - Данные
//	deferred_context - Структура - Данные
//
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
//
//DynamicDirective
Функция context_init_deferred(context, deferred_context)
	_context = BlocklyContext_init();
	_context.params = context.params;
	
     _context.block_context = context.block_context;
    _context.is_deferred = deferred_context["__deferred"];
    _context.variable_scopes = context.variable_scopes;
	_context.variable_scope_deferred_index = deferred_context.variable_scope_deferred_index;

   _context.data = context.data;
	 _context.operation = context.operation;
    _context.report = context.report;
...
Функция context_to_result()
	res = Новый Структура("ContextId, Result, Status, Commands", context.uid, context.result, context.status, context.commands);
	Если ЗначениеЗаполнено(context.debug_mode) Тогда
		res.Вставить("CurrentVariables", context.current_variables);
		res.Вставить("CurrentBlock", context.current_block);
		res.Вставить("CurrentWorkspace", context.current_workspace);
	КонецЕсли;
	Actions = get_prop(context.report, "actions_to_write");
	Если ЗначениеЗаполнено(Actions) Тогда 
		res.Вставить("Actions", Actions);	
	КонецЕсли;
	Возврат res;
КонецФункции
...
//
// Параметры:
//  block_id - Строка - Идентификатор блока
//
//DynamicDirective
...
//
//Параметры:
//  ПринудительныйВызов - Булево - Вызывать команды принудительно
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
//
//DynamicDirective
Функция context_check_command_limit(ПринудительныйВызов = Ложь)
	Если context["commands"].Количество() > 0  И (context["commands"].Количество() >= context["limit_commands"] Или ПринудительныйВызов) Тогда
		ВызватьИсключение NewExtExceptionСтрока(,"LimitCommand",,,,"LimitCommand");		
	КонецЕсли;
	Возврат Неопределено;
КонецФункции

// context_set_command_limit
//
// Параметры:
//  debug_mode - Булено - Признак отладки
//
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
//
...
// context_variable_scope_add
//
// Параметры:
//  context - Структура - Контекст исполняемого алгоритма
//  block_context - Структура - Контекст исполняемого блока
//  params - Стуктара - Дополнительные параметры
//
//DynamicDirective
Процедура context_variable_scope_add(context, block_context, params)
	Если get_prop(block_context, "_variable_scope") <> Неопределено Тогда
		context.variable_scope_current = block_context["_variable_scope"];
	Иначе
		context.variable_scopes.Добавить(params);
		context.variable_scope_current = context.variable_scopes.Количество() - 1;
		block_context.Вставить("_variable_scope", context.variable_scope_current)
	КонецЕсли;	
КонецПроцедуры

// context_variable_scope_remove
//
// Параметры:
//  context - Структура - Контекст исполняемого алгоритма
//  block_context - Стуктара - deferred_exception
//
//DynamicDirective
Процедура context_variable_scope_remove(context, block_context)
	Если block_context["_variable_scope"] <> context.variable_scope_current 
		Или context.variable_scopes.Количество() - 1 <> context.variable_scope_current Тогда
		ВызватьИсключение NewExtExceptionСтрока(,"Ошибка уровня вложенности variable_scope");
	КонецЕсли;
	context.variable_scopes.Удалить(context.variable_scope_current);
    context.variable_scope_current = context.variable_scope_current - 1;
КонецПроцедуры

// context_add_deferred
//
// Параметры:
//  context - Структура - Контекст исполняемого алгоритма
//  deferred_exception - Стуктара - deferred_exception
//
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
//
//DynamicDirective
Функция context_add_deferred(context, deferred_exception) 
	multi_thread_loop_block_context = deferred_exception.multi_thread_loop_block_context;
    multi_thread_loop_variable_scopes = deferred_exception.multi_thread_loop_variable_scopes;
	Попытка
		i = helper_find_in_array(context.deferred, deferred_exception.uuid, "__deferred");	
		context.variable_scope_deferred_index = context.deferred[i]["variable_scope_deferred_index"];	
	Исключение     
		context.deferred.Добавить(Новый Структура);
		i = context.deferred.Количество() - 1;
		Если context.variable_scope_deferred_index = Неопределено Тогда 
			context.variable_scope_deferred_index = context.variable_scope_current;
		КонецЕсли;
		Если get_prop(multi_thread_loop_block_context, "__deferred_count") = Неопределено Тогда 
			multi_thread_loop_block_context.Вставить("__deferred_count", 0);	
		КонецЕсли;	
	КонецПопытки;	
	context.deferred[i].Вставить("__deferred", 		deferred_exception.uuid);	
	context.deferred[i].Вставить("block_context", 	Saby_КопияСтруктуры(multi_thread_loop_block_context));	
	context.deferred[i].Вставить("variable_scopes", Saby_КопияМассива(multi_thread_loop_variable_scopes));	
	context.deferred[i].Вставить("variable_scope_deferred_index", context.variable_scope_deferred_index);	
	
	variable_scopes_enumerator = context.variable_scopes.Количество() - 1;
	Пока variable_scopes_enumerator > context.variable_scope_deferred_index Цикл
		context.variable_scopes.Удалить(variable_scopes_enumerator);
		variable_scopes_enumerator = variable_scopes_enumerator - 1;
	КонецЦикла;	
	context.variable_scope_current = context.variable_scope_deferred_index; 
	Возврат Неопределено;
КонецФункции

//DynamicDirective

Процедура context_restore_after_deferred(context, last_deferred_context)
	context.block_context = last_deferred_context.block_context;
	context.variable_scopes = last_deferred_context.variable_scopes;	
КонецПроцедуры


// Установить переменную в контексте
//
// Параметры:
//  request_exception - Стуктара - request_exception
//  block_context - Структура - Контекст исполняемого блока
//
// Возвращаемое значение:
...
Функция context_add_request(request_exception, block_context)
	...
	сз.ЗагрузитьЗначения(context.block_context.__thread_vars);
	block_context_request.Вставить("__thread_vars", сз.ВыгрузитьЗначения()); 
	Запись_QueryId = request_exception.dump; 
	Запись_QueryId.Вставить("block_context", block_context_request);
	ТранспортИнтеграции.async_requests.Вставить(QueryId, Запись_QueryId);
...
Функция context_variables_get(_context)
	Возврат _context.variable_scopes[_context.variable_scope_current]
КонецФункции
...
Функция add_new_context()
	save_last_context(); 
	report_ext = contexts[contexts.Количество()-1].report;
	...
	context.command_result	= contexts[contexts.Количество()-1].command_result;
	context.report	= report_init(report_ext.operation, report_ext.uuid);
	context.report.new = Истина;
	Возврат Неопределено;
КонецФункции
// Добавляет контекст в конец списка
//
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
//
...
Функция save_last_context()
	...
	Если BlocklyExecutor = Неопределено Тогда
		BlocklyExecutor = blockly_executor_init(,, Новый Соответствие);	
	КонецЕсли;
	workspaces.Добавить(BlocklyExecutor.workspace);
	contexts.Добавить(context);
	Возврат Неопределено;
КонецФункции
// Удаляет последний в списке контекст
//
// Возвращаемое значение:
//  - Неопределено - Результат обсчета алгоритма
//
...
Функция delete_last_context()
	...
	// BSLLS:UnusedLocalVariable-off Определяем глобальную переменную
	Workspace = get_prop(BlocklyExecutor.workspace, "data");
