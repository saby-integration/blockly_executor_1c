...
// BSLLS:UnusedLocalVariable-off  Переменная модуля
   Step = get_prop(ПараметрыВызова, "CurrentBlock");
   Selected = get_prop(ПараметрыВызова, "selected"); 
...
// BSLLS:UnusedLocalVariable-on
	...
	Если get_prop(get_prop(ПараметрыВызова, "EndpointArgs", Новый Структура), "Uuid") <> Неопределено Тогда
		operation_uuid = get_prop(ПараметрыВызова.EndpointArgs, "Uuid");
	Иначе
	...
	operation_uuid_default = get_prop(ПараметрыВызова, "КлючФоновогоЗадания");
	Report = report_init(operation, operation_uuid, , action_to_write, algorithm_name, operation_uuid_default);
	...
	Попытка
		...
		Если params = Неопределено Тогда
			params = НастройкиПодключенияПрочитать();			
		КонецЕсли;
	...
	integration_id = get_prop(context.params, "IntegrationId", "");
