...
//
// Возвращаемое значение:
//  - Неопределено или Строка - Результат обсчета алгоритма
//
//DynamicDirective
Функция workspace_find_comment(node)
	comment = Workspace.ВычислитьВыражениеXpath("./b:comment", node, размыватель).ПолучитьСледующий();
	Если comment <> Неопределено Тогда
		Возврат comment.ТекстовоеСодержимое;
	КонецЕсли;
	Возврат Неопределено;
КонецФункции
...
Функция workspace_execute_inputs(node, path, context, block_context)
	...
	Пока Истина Цикл
		...
		Если Узел = Неопределено Тогда 
			Прервать; 
		КонецЕсли;
		Если Не block_context.Свойство(Узел.ПолучитьАтрибут("name")) Тогда
			РассчитатьВыполнениеВхождения(Узел, node, path, context, block_context);
		КонецЕсли;
	КонецЦикла;
	Возврат Неопределено;
КонецФункции
...
//DynamicDirective
Процедура РассчитатьВыполнениеВхождения(Узел, node, path, context, block_context)
...
Функция workspace_get_start_block(endpoint, context)
	...
	Если ЗначениеЗаполнено(endpoint) Тогда
		...
		Если block = Неопределено Тогда
			ВызватьИсключение NewExtExceptionСтрока(, "Не найден endpoint", 
				"В алгоритме " + context.workspace_name + " не найден endpoint " + endpoint, , ,"NotFound");
		КонецЕсли;
	Иначе
